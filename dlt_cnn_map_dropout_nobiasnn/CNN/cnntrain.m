function net = cnntrain(net, x, y, opts)
    ...
    for i = 1 : opts.numepochs
        ...
        tic;
        kk = randperm(m);                       %把1到m这些数字随即打乱成一个数字序列

        for l = 1 : numbatches
            waitbar((l+(i-1)*numbatches)/ opts.numepochs * numbatches, hwait, sprintf('正在训练:%d/%d @epcoh: %d/%d', l,numbatches,i,opts.numepochs));
